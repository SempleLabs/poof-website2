...
**Customer deposits are a liability, not revenue.** Money taken up front on a generator install before any work is done is not yours yet. In Poof Managed Bookkeeping that is the default: the deposit sits as a liability and is released to the job's revenue when the install is completed.
...
*Written by Austin Semple, a former controller: three years in audit, seven running the books for small businesses, and the founder of Poof. If you'd rather have the structure built for you and maintained every month — with per-job P&L on every close, reviewed by a controller you can text — [that's what Poof Managed Bookkeeping is](/electrical).*
