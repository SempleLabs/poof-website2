...
**Customer deposits are a liability, not revenue.** Money taken up front on a generator install before any work is done is not yours yet. In Poof for shops that is the default: the deposit sits as a liability and is released to the job's revenue when the install is completed.
...
One caveat before you start: a clean structure on top of messy history does not give you clean books. If your last few months are unreconciled or miscategorized, fix that first — the [bookkeeping cleanup checklist](/blog/tax-season-bookkeeping-checklist) walks through the order to do it in.

A second caveat, and it is the one that undoes good structures quietly: if you use QuickBooks bank rules to keep the new accounts filled, write them tightly and check quarterly what they actually caught. A rule auto-posts from the moment you save it and keeps applying whether or not it is right, so one rule written a shade too broadly can fill a brand-new account with the wrong transactions for months. That risk, and what it costs to run QuickBooks Plus at $140/mo for job costing in the first place, are covered in [what to do when QuickBooks gets too expensive](/blog/quickbooks-too-expensive-alternatives).
...
*Written by Austin Semple, a former controller: three years in audit, seven running the books for small businesses, and the founder of Poof. Poof sets this structure up for you and keeps it — with per-job P&L on every close, and every AI entry staged for you to approve: [that's what Poof for shops is](/trades).*
